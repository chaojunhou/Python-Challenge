...
text = '高高我爱你'  #输入要打得水印
im = Image.open('D://gaogao.JPG')                         #打开要打水印的相片

imageW,imageH = im.size         #获取图片尺寸
textImageW = int(imageW*1.9)    #确定写文字图片的尺寸，如前所述，要比照片大，我取1.5倍
textImageH = int(imageH*1.9)
...
if imageW < 400:
    ...
    k = 300
print "fontsize:",k
Font = ImageFont.truetype("D:\SHOWG.TTF",k)  #创建Font对象，k之为字号
textW,textH = Font.getsize(text)
print textW,textH
d.ink = 255 + 0 * 256 + 0 * 256 * 256         #黑色
...
cropH = rLen*math.sin(oriAngle + math.pi/5) *2
box = [int((textImageW-cropW)/2-50),int((textImageH-cropH)/2-1)-50,int((textImageW+cropW)/2+20),int((textImageH+cropH)/2+20)]
...
textBlank.save('gao.jpg','JPEG')
waterImage = Image.blend(im,textBlank,0.2)
waterImage.save('gaogao.jpg','JPEG')
waterImage.show()
